...
const refreshAuthToken = async () => {
  ...
  try {
    ...
    if (data.data.accessToken) {
      // set expiry to 30d, and 60d for refresh token
...
export const realApi = {
  ...
  orgs: {
    ...
    updateMemberRole: async (orgId, userId, role) => {
      return fetchWithAuth(`/orgs/${orgId}/members/${userId}`, {
        method: "PUT",
    ...
    getStats: async (orgId) => {
      return fetchWithAuth(`/orgs/${orgId}/stats`);
    },
  },
  ...
  sessions: {
    ...
    getTestResults: async (sessionId, filters = {}) => {
      const params = new URLSearchParams();
      if (filters.testerIds && filters.testerIds.length > 0) {
        params.append('testerIds', filters.testerIds.join(','));
      }
      if (filters.status && filters.status.length > 0) {
        params.append('status', filters.status.join(','));
      }
      const queryString = params.toString();
      return fetchWithAuth(`/sessions/${sessionId}/results${queryString ? `?${queryString}` : ''}`);
    },
    ...
    assignUser: async (sessionId, userId) => {
      return fetchWithAuth(`/sessions/${sessionId}/assign`, {
        method: "POST",
  ...
  cases: {
    ...
    bulkCreate: async (featureId, cases) => {
      return fetchWithAuth(`/features/${featureId}/cases/bulk`, {
        method: "POST",
  ...
  invitations: {
    ...
    getOrganizationInvitations: async (orgId) => {
      return fetchWithAuth(`/invitations/organizations/${orgId}/invitations`);
    },
  ...
  // AI endpoints
  ai: {
    generateTestCases: async (inputText, model = "bytez-kei", options = {}) => {
      return fetchWithAuth("/ai/generate-test-cases", {
        method: "POST",
        body: JSON.stringify({ inputText, model, options }),
      });
    },

    getAvailableModels: async () => {
      return fetchWithAuth("/ai/models");
    },
  },
  ...
  issues: {
    getAll: async (orgId, filters = {}) => {
      ...
      return fetchWithAuth(`/orgs/${orgId}/issues?${queryString}`);
    },

    getById: async (issueId) => {
      return fetchWithAuth(`/issues/${issueId}`);
    },

    create: async (orgId, issueData) => {
      return fetchWithAuth(`/orgs/${orgId}/issues`, {
        method: "POST",
    ...
    updateStatus: async (issueId, statusData) => {
      return fetchWithAuth(`/issues/${issueId}/status`, {
        method: "PATCH",
    ...
    assign: async (issueId, data) => {
      return fetchWithAuth(`/issues/${issueId}/assign`, {
        method: "POST",
        body: JSON.stringify(data),
      });
    },

    addComment: async (issueId, data) => {
      return fetchWithAuth(`/issues/${issueId}/comments`, {
        method: "POST",
        body: JSON.stringify(data),
      });
    },
    ...
    updateComment: async (issueId, commentId, data) => {
      return fetchWithAuth(`/issues/${issueId}/comments/${commentId}`, {
        method: "PUT",
        body: JSON.stringify(data),
      });
    },
    ...
    vote: async (issueId, data) => {
      return fetchWithAuth(`/issues/${issueId}/vote`, {
        method: "POST",
        body: JSON.stringify(data),
      });
    },
    ...
    toggleWatch: async (issueId) => {
      return fetchWithAuth(`/issues/${issueId}/watch`, {
        method: "POST",
      });
    },

    addImages: async (issueId, data) => {
      return fetchWithAuth(`/issues/${issueId}/images`, {
        method: "POST",
        body: JSON.stringify(data),
      });
    },

    removeImage: async (issueId, imageId) => {
      return fetchWithAuth(`/issues/${issueId}/images/${imageId}`, {
        method: "DELETE",
      });
    },

    generateAITicket: async (issueId) => {
      return fetchWithAuth(`/issues/${issueId}/ai/generate-ticket`, {
        method: "POST",
      });
    },

    analyzeIssue: async (issueId) => {
      return fetchWithAuth(`/issues/${issueId}/ai/analyze`, {
        method: "POST",
      });
    },

    notifyMembers: async (issueId, data) => {
      return fetchWithAuth(`/issues/${issueId}/notify`, {
        method: "POST",
        body: JSON.stringify(data),
      });
    },

    notifyMembersMultiple: async (orgId, data) => {
      return fetchWithAuth(`/orgs/${orgId}/issues/notify-batch`, {
        method: "POST",
        body: JSON.stringify(data),
      });
    },
    ...
    getStats: async (orgId) => {
      return fetchWithAuth(`/orgs/${orgId}/issues/stats`);
    },
  },
  ...
  jira: {
    getConfig: async (orgId) => {
      return fetchWithAuth(`/jira/orgs/${orgId}/config`);
    },

    saveConfig: async (orgId, configData) => {
      return fetchWithAuth(`/jira/orgs/${orgId}/config`, {
        method: "POST",
    ...
    updateConfig: async (orgId, configData) => {
      return fetchWithAuth(`/jira/orgs/${orgId}/config`, {
        method: "PUT",
        body: JSON.stringify(configData),
      });
    ...
    testConnection: async (configData) => {
      return fetchWithAuth(`/jira/orgs/${configData.orgId || 'test'}/test-connection`, {
        method: "POST",
        body: JSON.stringify(configData),
      });
    },

    createTicket: async (issueId, data) => {
      return fetchWithAuth(`/jira/issues/${issueId}/create`, {
        method: "POST",
        body: JSON.stringify(data),
      });
    },

    syncTicket: async (issueId) => {
      return fetchWithAuth(`/jira/issues/${issueId}/sync`, {
        method: "POST",
      });
    },

    unlinkTicket: async (issueId) => {
      return fetchWithAuth(`/jira/issues/${issueId}/unlink`, {
        method: "DELETE",
      });
    },

    getProjects: async (configData) => {
      return fetchWithAuth(`/jira/projects`, {
        method: "POST",
        body: JSON.stringify(configData),
      });
    },

    getIssueTypes: async (configData) => {
      return fetchWithAuth(`/jira/issue-types`, {
        method: "POST",
        body: JSON.stringify(configData),
      });
    },

    getAssignableUsers: async (configData) => {
      return fetchWithAuth(`/jira/assignable-users`, {
        method: "POST",
        body: JSON.stringify(configData),
      });
    },

    searchUsers: async (configData, query) => {
      return fetchWithAuth(`/jira/search-users?query=${encodeURIComponent(query)}`, {
        method: "POST",
        body: JSON.stringify(configData),
      });
    },

    getProjectMembers: async (configData) => {
      return fetchWithAuth(`/jira/project-members`, {
        method: "POST",
        body: JSON.stringify(configData),
      });
    },
