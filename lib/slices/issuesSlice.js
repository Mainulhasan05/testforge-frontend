...
export const unlinkJiraTicket = createAsyncThunk(
  'issues/unlinkJira',
  async (issueId, { rejectWithValue }) => {
    try {
      const response = await realApi.jira.unlinkTicket(issueId);
      return response;
    } catch (error) {
      return rejectWithValue(error.message);
    }
...
const issuesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    // Unlink Jira ticket
    builder
      .addCase(unlinkJiraTicket.pending, (state) => {
        state.jiraUnlinking = true;
      })
      .addCase(unlinkJiraTicket.fulfilled, (state) => {
        state.jiraUnlinking = false;
        if (state.currentIssue) {
          state.currentIssue.jiraTicket = null;
        }
      })
      .addCase(unlinkJiraTicket.rejected, (state, action) => {
        state.jiraUnlinking = false;
        state.error = action.payload;
      });
