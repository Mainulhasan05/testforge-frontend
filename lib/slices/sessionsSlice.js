...
export const unassignUserFromSession = createAsyncThunk(
  ...
  async ({ sessionId, userId }) => {
    const response = await realApi.sessions.unassignUser(sessionId, userId);
    return { sessionId, userId, data: response.data };
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(assignUserToSession.fulfilled, (state, action) => {
        if (state.currentSession?._id === action.payload.data._id) {
          state.currentSession = action.payload.data;
        }
      })
      // Unassign user from session
      .addCase(unassignUserFromSession.fulfilled, (state, action) => {
        if (state.currentSession?._id === action.payload.data._id) {
          state.currentSession = action.payload.data;
        }
      });
