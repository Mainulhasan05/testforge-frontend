...
export const logout = createAsyncThunk("auth/logout", async () => {
  // Just clear local state, don't call API to avoid "No token" errors
  // The token removal happens in the reducer
  return null;
});

export const fetchMe = createAsyncThunk("auth/fetchMe", async () => {
  const response = await realApi.auth.getMe();
  return response.data;
});
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(login.fulfilled, (state, action) => {
        ...
        Cookie.set("authRefreshToken", action.payload.refreshToken);
      ...
      .addCase(fetchMe.fulfilled, (state, action) => {
        ...
        Cookie.set("authUser", JSON.stringify(action.payload));
      });
  },
});
