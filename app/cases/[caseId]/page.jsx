...
import { fetchCaseById } from "@/lib/slices/casesSlice"
...
export default function CaseDetailPage() {
  ...
  const { list: feedback, status: feedbackStatus, meta } = useSelector((state) => state.feedback)
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <Badge variant={getStatusColor(currentCase.status)}>{currentCase.status}</Badge>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  <h3 className="text-sm font-medium mb-2">Test Steps / Notes</h3>
                  <p className="text-sm text-muted-foreground">{currentCase.note || "No notes provided"}</p>
                </div>
                <div>
                  <h3 className="text-sm font-medium mb-2">Expected Output</h3>
                  <p className="text-sm text-muted-foreground">
                    {currentCase.expectedOutput || "No expected output specified"}
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Test results and comments from team members</CardDescription>
              </CardHeader>
              <CardContent>
                      ...
                      <div key={item.id} className="flex gap-4 pb-4 border-b last:border-0">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <Badge variant={item.result === "pass" ? "secondary" : "destructive"} className="text-xs">
                              {item.result === "pass" ? (
                            ...
                            <span className="text-xs text-muted-foreground">{formatLocalDateTime(item.createdAt)}</span>
                          </div>
